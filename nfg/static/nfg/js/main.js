...
/* funzioni di disegno per il menu */
...
function drawNewNF(){
     ele = {
        ...
        group.selectAll(".interface")
                ...
                .attr("cy","10")
                .attr("r",r_interface)
                .attr("parent_NF_position_x",ele.x)
                .attr("parent_NF_position_y",ele.y)
                .attr("id",function(e){return "vnf:"+ele.id+":"+e.id;})
                .call(drag_INTERFACE);
        
        group.attr("x",ele.x);
        group.attr("y",ele.y);
        group.attr("transform","translate("+ele.x+","+ele.y+")");
...
function drawEndPointInfo(endpoint,id){
    $('.info').empty();
    $('.info').append('<a href="#"><i class="glyphicon glyphicon-exclamation-sign"></i><strong> Node Info</strong></a><div class="panel panel-default"><div class="panel-heading"><a>Node Id: '+endpoint.id+' </a></div><div id="end'+endpoint.id+'"class="panel-body"><p><b>Name:</b> '+endpoint.name+'</p><p><b>Type:</b> '+endpoint.type+'</p></div></div>');
