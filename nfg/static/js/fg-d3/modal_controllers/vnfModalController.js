...
(function () {
     ...
     * @param templates
     */
    var editVNFModalController = function ($uibModalInstance, fg, fgPos, schema, templates) {
        var ctrl = this;
        ...
        ctrl.templateChanged = function () {
            ...
            if (filtered.length > 0) {
                ...
                for (var i = 0; i < ctrl.VNFProperties.ports.length; i++) {
                    ...
                    var max = maxId == "N" ? Number.MAX_VALUE : Number(maxId) - Number(minId) + 1;
                    var j = 0;
                    for (var k = 0; k < ctrl.fgElem.ports.length; k++) {
                        if (ctrl.fgElem.ports[k].id.split(":")[0] == ctrl.VNFProperties.ports[i].label)
                            j++;
                    }
                    ctrl.vnfPortInfo.push({
                        label: ctrl.VNFProperties.ports[i].label,
        ...
        ctrl.addPort = function (portInfo) {
            ...
            if (id == -1)
                id = portInfo.minId + portInfo.tot;
            var portID = portInfo.label + ":" + id;
            ctrl.fgElem.ports.push({id: portID});
            ctrl.fgPosElem.ports[portID] = {
                ref: "VNF_interface",
                id: portID,                                        // id of the port, unique in vnf
                full_id: "vnf:" + ctrl.fgElem.id + ":" + portID,   // id unique across all elements
                parent_vnf_id: ctrl.fgElem.id,                    // id of the vnf associated
                isLinked: false
            };
        ...
        ctrl.removePort = function (port, portInfo) {
            ...
            delete ctrl.fgPosElem.ports[port.id];
    ...
    editVNFModalController.$inject = ['$uibModalInstance', 'fg', 'fgPos', 'schema', 'templates'];
